...
namespace YaSha.DataManager.ListProcessing.PackageSheet
{
    internal class PackSheetRule
    {
        ...
        internal static List<PackSheetRule> Get(string path)
        {
            List<PackSheetRule> res = new List<PackSheetRule>();

            using (ExcelPackage package = new ExcelPackage(path))
            {
                var worksheet = package.Workbook.Worksheets["包装单-修改设置"];
                ...
                for (int row = 2; row <= rows; row++)
                {
                    ...
                    PackSheetRule data = new PackSheetRule
                    {
    ...
    internal class PackSheetFilterRule
    {
        internal string 序号 { get; set; }
        ...
        internal static List<PackSheetFilterRule> Get(string path)
        {
            List<PackSheetFilterRule> res = new List<PackSheetFilterRule>();

            using (ExcelPackage package = new ExcelPackage(path))
            {
                var worksheet = package.Workbook.Worksheets["包装单-过滤设置 "];
                ...
                for (int row = 2; row <= rows; row++)
                {
                    var data = new PackSheetFilterRule();
        ...
        internal static List<OriginData> Filter(List<OriginData> originDatas, List<PackSheetFilterRule> filterRules)
        {
        ...
        static List<List<PackSheetFilterRule>> GetRules(List<PackSheetFilterRule> rules)
        {
            List<List<PackSheetFilterRule>> results = new List<List<PackSheetFilterRule>>();
            ...
            for (int i = 0; i < main.Count; i++)
            {
                if (i == main.Count - 1)
                {
                    ...
                    {
                        results.Add(new List<PackSheetFilterRule>() { main[i] });
                    }
                    else
                    {
                        var tmp = new List<PackSheetFilterRule>();
    ...
    internal class PackSheetSortRule
    {
        internal string 字段 { get; set; }

        internal string 排序规则 { get; set; }

        internal static List<PackSheetSortRule> Get(string path)
        {
            List<PackSheetSortRule> res = new List<PackSheetSortRule>();

            using (ExcelPackage package = new ExcelPackage(path))
            {
                var worksheet = package.Workbook.Worksheets["包装单-排序设置"];
                ...
                for (int row = 2; row <= rows; row++)
                {
                    var data = new PackSheetSortRule();
        ...
        internal static List<OriginData> Sort(List<OriginData> datas, List<PackSheetSortRule> rules)
        {
            IOrderedEnumerable<OriginData> tmp = null;

            foreach (var item in rules)
            {
                if (rules.IndexOf(item) == 0)
                {
                    if (item.排序规则 == "升序")
                    {
                        tmp = datas.OrderBy(x =>
