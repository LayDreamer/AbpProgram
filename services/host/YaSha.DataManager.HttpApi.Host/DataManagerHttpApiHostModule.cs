...
namespace YaSha.DataManager
{
    ...
    public class DataManagerHttpApiHostModule : AbpModule
    {
                ...
                .AddJwtBearer(options =>
                {
                    ...
                    options.Events = new JwtBearerEvents
                    {
                        OnMessageReceived = currentContext =>
                        {
                            ...
                            if (accessToken.IsNullOrWhiteSpace())
                            {
                                accessToken = currentContext.Request.Cookies[DataManagerHttpApiHostConst.DefaultCookieName];
                            }
        ...
        private void ConfigureCache(ServiceConfigurationContext context)
        {
            Configure<AbpDistributedCacheOptions>(
                options => { options.KeyPrefix = "DataManager:"; });
            var configuration = context.Services.GetConfiguration();
            var enabled = configuration.GetValue("Redis:Enabled", false);
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            context.Services
                .AddDataProtection()
                .PersistKeysToStackExchangeRedis(redis, "DataManager-Protection-Keys");
        }
        ...
        private void ConfigurationSignalR(ServiceConfigurationContext context)
        {
            ...
            context.Services.AddSignalR().AddStackExchangeRedis(redisConnection, options => { options.Configuration.ChannelPrefix = "YaSha.DataManager"; });
        }
        ...
        private void ConfigureSwaggerServices(ServiceConfigurationContext context)
        {
            context.Services.AddSwaggerGen(
                options =>
                {
                    ...
                    options.SwaggerDoc("DataManager",
                        new OpenApiInfo { Title = "DataManager API", Version = "v1" });
        ...
        private void ConfigureCap(ServiceConfigurationContext context)
        {
            ...
            if (enabled)
            {
                context.AddAbpCap(capOptions =>
                {
                    ...
                    capOptions.UseEntityFramework<DataManagerDbContext>();
                    ...
                    capOptions.UseDashboard(options =>
                    {
                        ...
                        options.AuthorizationPolicy = DataManagerCapPermissions.CapManagement.Cap;
                    });
                });
            }
            else
        ...
        private void ConfigureAuditLog(ServiceConfigurationContext context)
        {
            ...
            (
                options =>
                {
                    ...
                    options.ApplicationName = "YaSha.DataManager";
                }
            );
