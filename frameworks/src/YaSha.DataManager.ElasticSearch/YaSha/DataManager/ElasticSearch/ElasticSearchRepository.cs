    ...
    where TEntity : class, IElasticSearchEntity
{
    ...
    /// 根据id查询实体
    /// </summary>
    /// <exception cref="DataManagerElasticSearchEntityNotFoundException"></exception>
    ...
    public virtual async Task<TEntity> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.IsValid)
        {
            throw new DataManagerElasticSearchEntityNotFoundException(innerException: result.OriginalException);
        }
    ...
    /// </summary>
    /// <exception cref="DataManagerElasticSearchException"></exception>
    public virtual async Task InsertAsync(TEntity entity, CancellationToken cancellationToken = default)
    {
        var result = await Client.IndexAsync(entity, x => x.Index(IndexName), cancellationToken);
        if (!result.IsValid)
        {
            throw new DataManagerElasticSearchException(innerException: result.OriginalException);
        }
    }
