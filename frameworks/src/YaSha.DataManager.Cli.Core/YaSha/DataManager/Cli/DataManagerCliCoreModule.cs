...
public class DataManagerCliCoreModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        Configure<Options.DataManagerCliOptions>(options =>
        {
            ...
            options.Templates = new List<DataManagerTemplateOptions>()
            {
                new DataManagerTemplateOptions("abp-vnext-pro", "pro", "源码版本", true)
                {
                    ExcludeFiles = "templates,docs,.github,LICENSE,Readme.md",
                    ReplaceSuffix = ".sln,.csproj,.cs,.cshtml,.json,.ci,.yml,.yaml,.nswag,.DotSettings,.env,Directory.Build.YaSha.targets",
                    OldCompanyName = "YaSha",
                    OldProjectName = "DataManager"
                },
                new DataManagerTemplateOptions("abp-vnext-pro-nuget-all", "pro.all", "Nuget完整版本")
                {
                ...
                new DataManagerTemplateOptions("abp-vnext-pro-nuget-simplify", "pro.simplify", "Nuget简单版本")
                {
                ...
                new DataManagerTemplateOptions("abp-vnext-pro-nuget-module", "pro.module", "模块")
                {
