namespace YaSha.DataManager.Cli.SourceCode;
...
public class SourceCodeManager : ITransientDependency, ISourceCodeManager
{
    ...
    private readonly DataManagerCliOptions _cliOptions;

    public SourceCodeManager(ILogger<SourceCodeManager> logger, IOptions<Options.DataManagerCliOptions> options, IDataManagerManager abpProManager)
    {
        _logger = logger;
    ...
    public async Task<TemplateFile> GetAsync(string version)
    {
        ...
        _logger.LogInformation($"YaSha DataManager Version:{version}");
